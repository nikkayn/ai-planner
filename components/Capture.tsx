...
export default function Capture({ onTasksReady }: Props) {
  ...
  const accumulatedTasksRef = useRef<Task[]>([])
  const recognitionRef = useRef<any>(null)
  const bufferRef = useRef('')

  const startListening = useCallback(() => {
    const SR = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition
    if (!SR) { alert('Голосовий ввід не підтримується. Використовуйте текстове поле.'); return }
  ...
  const handleTransform = async () => {
    ...
    try {
      ...
      setTimeout(() => { onTasksReady(accumulatedTasksRef.current) }, 800)
    } finally {
      setIsLoading(false)
    }
  }

  const priorityColor: Record<string,string> = { high: 'bg-red-500', medium: 'bg-yellow-500', low: 'bg-green-500' }
    ...
    <div className="flex flex-col h-full p-4 gap-4">
      ...
      <div className="flex justify-center">
        <button onClick={isListening ? stopListening : startListening}
          className={`w-20 h-20 rounded-full flex items-center justify-center text-3xl transition-all ${isListening ? 'bg-red-500 animate-pulse scale-110' : 'bg-indigo-600 hover:bg-indigo-500'}`}>
          {isListening ? '⏹' : '🎤'}
        </button>
      </div>
      {isListening && <p className="text-center text-red-400 text-sm animate-pulse">Слухаю...</p>}
      <textarea value={text} onChange={e => setText(e.target.value)}
        placeholder="або пишіть тут..."
      ...
      <button onClick={handleTransform} disabled={isLoading || !text.trim()}
      ...
      {streamedTasks.length > 0 && (
        <div className="flex flex-col gap-2 mt-2">
          ...
          {streamedTasks.map(task => (
            <div key={task.id} className="flex items-center gap-3 bg-gray-900 rounded-xl px-3 py-2 animate-fade-in">
