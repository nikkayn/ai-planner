...
export default function Capture({ onTasksReady }: Props) {
  ...
  const startListening = useCallback(() => {
    ...
    if (!SR) {
      alert('Голосовий ввід не підтримується. Використовуйте текстове поле.')
      return
    }
  ...
  return (
    <div className="flex flex-col h-full p-4 gap-4">
      ...
      <div className="flex justify-center">
        <button
          ...
          className={`w-20 h-20 rounded-full flex items-center justify-center text-3xl transition-all ${
            isListening ? 'bg-red-500 animate-pulse scale-110' : 'bg-indigo-600 hover:bg-indigo-500'
          }`}
        >
          {isListening ? '⏹' : '🎤'}
        </button>
      ...
      <textarea
        ...
        placeholder="або пишіть тут... зустріч з Максом о 3, купити продукти..."
